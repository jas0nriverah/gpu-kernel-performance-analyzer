...
I time CUDA kernels with CUDA events using warmups and repeated trials, then report a full
distribution plus a coefficient of variation so I know the measurement is stable. From each
kernel's declared bytes and FLOPs I derive bandwidth, GFLOPs, and arithmetic intensity, and
I plot them on a roofline to classify kernels as memory- or compute-bound. The headline
result is tiled versus naive GEMM: tiling stages data in shared memory, raises arithmetic
intensity, and gives a measured speedup. On top of the measured data I train a small model
to predict runtime and recommend block sizes, which is the autotuning problem in miniature.
The part I care most about is provenance: every value is tagged measured, derived,
predicted, or unavailable, and the validator refuses to call a profiler metric measured
unless real Nsight data was imported for that exact scenario, so the tool cannot overclaim.
...
- Parameterized GEMM tile sizes (currently fixed at 16x16) and a coarsened or `float4` variant.
- Auto-fill roofline ceilings from device metadata where peak specs are reliably queryable.
- Multi-run comparison and regression tracking across git commits.
