...
def _load_gpu_telemetry_csv(
    ...
    if "session_id" in columns:
        out["session_id"] = clean["session_id"].astype(str)
    elif job_col:
    ...
    out["workload_type"] = clean["workload_type"] if "workload_type" in columns else f"{source_name}_gpu_trace"
    for identity in ("gpu_uuid", "gpu_name", "run_id", "scenario_id", "trial", "backend", "power_scope"):
        if identity in columns:
            out[identity] = clean[identity]
