...
def get_train_test_indices(
    ...
    if split_strategy == SPLIT_TIME and ("timestamp" not in df or pd.to_datetime(df["timestamp"], errors="coerce").isna().any()):
        raise ValueError("Time splitting requires valid timestamps for every row.")
    if split_strategy == SPLIT_GROUPED_SESSION and ("session_id" not in df or df["session_id"].isna().any() or df["session_id"].nunique() < 2):
        raise ValueError("Grouped splitting requires at least two non-null sessions.")
    if not 0 < test_size < 1 or len(df) < 2:
        raise ValueError("Split requires at least two rows and test_size between zero and one.")
    n = len(df)
    all_idx = np.arange(n)
